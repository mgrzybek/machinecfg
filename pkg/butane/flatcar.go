...
func CreateFlatcars(client *netbox.APIClient, ctx context.Context, filters commonFilters.DeviceFilters) (result []Flatcar, err error) {
	...
	case len(filters.Tenants) > 0 && filters.Tenants[0] != "" && len(filters.Locations) > 0 && filters.Locations[0] != "":
		...
		devices, response, err = client.DcimAPI.DcimDevicesList(ctx).HasPrimaryIp(true).Site(filters.Sites).Location(filters.Locations).Tenant(filters.Tenants).Role(filters.Roles).Execute()
	case len(filters.Tenants) > 0 && filters.Tenants[0] != "":
		slog.Info("CreateFlatcars", "message", "tenants")
		devices, response, err = client.DcimAPI.DcimDevicesList(ctx).HasPrimaryIp(true).Site(filters.Sites).Tenant(filters.Tenants).Role(filters.Roles).Execute()
	case len(filters.Locations) > 0 && filters.Locations[0] != "":
		slog.Info("CreateFlatcars", "message", "locations")
		devices, response, err = client.DcimAPI.DcimDevicesList(ctx).HasPrimaryIp(true).Site(filters.Sites).Location(filters.Locations).Role(filters.Roles).Execute()
	default:
		devices, response, err = client.DcimAPI.DcimDevicesList(ctx).HasPrimaryIp(true).Site(filters.Sites).Role(filters.Roles).Execute()
	}
