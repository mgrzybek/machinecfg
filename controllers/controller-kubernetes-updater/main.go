...
import (
	...
	"machinecfg/internal/controller/kubernetes_updater"
)
...
func main() {
	...
	// LEADER_ELECT defaults to true; set to "false" to disable (e.g. local dev).
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:        "kubernetes-updater.machinecfg",
	...
	cfg := kubernetes_updater.NewConfig()
	...
	if cfg.GetOtelEnabled() {
		shutdown, otelErr := kubernetes_updater.InitOtel(context.Background(), cfg.GetOtelEndpoint())
		if otelErr != nil {
	...
	if err := (&kubernetes_updater.ConfigReconciler{
	...
	// NetBoxSyncReconciler: polls NetBox and reconciles Hardware objects.
	if err := (&kubernetes_updater.NetBoxSyncReconciler{
		Client:    mgr.GetClient(),
		Config:    cfg,
		Namespace: namespace,
	}).SetupWithManager(mgr); err != nil {
		slog.Error("cannot setup netbox sync reconciler", "error", err.Error())
		os.Exit(1)
	}
	...
	slog.Info("starting machinecfg-controller-kubernetes-updater",
		"namespace", namespace,
		"backend", cfg.GetBackend(),
		"sync_interval", cfg.GetSyncInterval(),
